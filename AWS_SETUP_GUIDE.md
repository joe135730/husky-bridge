...
### Create Security Groups (You need TWO):

#### Security Group 1: ALB Security Group
1. Go to **EC2** → **Security Groups** → **Create security group**
2. Name: `husky-bridge-alb-sg`
3. Description: `Security group for Application Load Balancer`
4. Inbound rules:
   - Port 80 (HTTP) from `0.0.0.0/0` ✅ **Correct for ALB**
   - Port 443 (HTTPS) from `0.0.0.0/0` ✅ **Correct for ALB**
   - (Port 4000 is NOT needed here - ALB handles routing)
5. Note the Security Group ID: `sg-ALB-XXXXX`

#### Security Group 2: ECS Task Security Group
1. Create another security group
2. Name: `husky-bridge-ecs-sg`
3. Description: `Security group for ECS tasks`
4. Inbound rules:
   - Port 80 (Frontend) from ALB security group (`sg-ALB-XXXXX`) ⚠️ **NOT 0.0.0.0/0**
   - Port 4000 (Backend) from ALB security group (`sg-ALB-XXXXX`) ⚠️ **NOT 0.0.0.0/0**
5. Note the Security Group ID: `sg-ECS-XXXXX`

**Important**: ECS tasks should NOT be directly accessible from the internet. They should only accept traffic from the ALB!
...
## 📦 Step 5: Create ECS Service-Linked Role (REQUIRED FIRST!)

**IMPORTANT**: You must create this role BEFORE creating the ECS cluster!

### Using AWS Console:
1. Go to **IAM** → **Roles** → **Create role**
2. Select **AWS service**
3. Choose **Elastic Container Service**
4. Select **Elastic Container Service** (use case)
5. Click **Next** → **Create role**
6. The role will be automatically named: `AWSServiceRoleForECS`
...
**Note**: If you get an error saying the role already exists, that's fine - it means it's already created.
